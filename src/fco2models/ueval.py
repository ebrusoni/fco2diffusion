...
def load_model(save_dir, model_path, model_class, training_complete=True):
...
def plot_samples(ax, samples, truth, title, only_mean=False):
    """Plot samples and truth values generated by the given model."""
    ax.plot(samples.T, color='blue', alpha=0.5)
    if not only_mean:
        ax.plot(samples.mean(axis=0), '^-', label='reconstructed fco2', color='black', linewidth=2)
    ...
    ax.plot(truth.flatten(), '-o', label='true fco2', color='red', linewidth=2)
    ax.set_title(title)
...
def get_error_stats(samples, truth):
    ...
    samples: (n_samples, n_rec, n_bins), n_rec can also be 1
