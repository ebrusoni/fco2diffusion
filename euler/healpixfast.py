...
def fdf_to_numpy(df, cols, nside, x_lim=np.nan, y_lim=np.nan):
    fnpix = df.shape[0]
...
def do_random_rot(lon, lat):
        random_zrotation = np.random.random() * 10
...
def infer_patch_with_rotations(model, noise_scheduler, params, date, nside=1024, jump=None, n_samples=1):
    ...
    for i in range(n_samples):
        ...
        for t in t_loop:
            ...
            for face in range(7,8):
                #print("NONONONO")
                fnpix = npix // 12 
                fpixs = np.arange(face * fnpix, (face + 1) * fnpix)
                fpixs_rotated = m_rot[fpixs]
                
                df = cds[fpixs_rotated, colixs[:, np.newaxis]].T
                ...
                cds[fpixs_rotated, i] = samples[x, y] if step % 2 == 0 else samples[y, x]
                t_loop.set_postfix({"face": {face}, "sample": {i}})
            step += 1
    print(f"cds samples shape: {cds[:, :n_samples].shape}")
    print(f"context df samples shape: {context_df[sample_cols].shape}")
...
df.to_parquet(f'{save_path}papagayo.pq')