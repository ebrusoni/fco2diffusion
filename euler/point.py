...
predictors = ['sst_cci', 'sss_cci', 'chl_globcolour', 'ssh_sla', 'mld_dens_soda', 'lon', 'lat', 'day_of_year', 'xco2']
...
# flatten data to 2d, where each row is a single point
point_train_ds =  np.zeros((train_ds.shape[0]* train_ds.shape[2], train_ds.shape[1] ))
point_val_ds =  np.zeros((val_ds.shape[0] * val_ds.shape[2], val_ds.shape[1]))
for i in range(train_ds.shape[1]):
    point_train_ds[:, i] = train_ds[:, i, :].flatten()
    point_val_ds[:, i] = val_ds[:, i, :].flatten()

# drop all rows with nan values in the first column
point_train_ds = point_train_ds[~np.isnan(point_train_ds[:, 0])]
point_val_ds = point_val_ds[~np.isnan(point_val_ds[:, 0])]
#  print shapes
print(f"point_train_ds shape: {point_train_ds.shape}")
print(f"point_val_ds shape: {point_val_ds.shape}")


train_dataset = TensorDataset(torch.tensor(point_train_ds))
val_dataset = TensorDataset(torch.tensor(point_val_ds))
